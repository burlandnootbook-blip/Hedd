#!/usr/bin/env python3
"""
Shopify Card Checker – Windows RDP version (headed browser)
"""
...
async def check_card(card_line):
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False)  # Headed mode for RDP
        ...
        # Go to cart
        print("[*] Going to cart...")
        await page.goto(BASE_URL + "/cart", wait_until="networkidle")
        await page.wait_for_timeout(2000)

        # Click checkout
        print("[*] Clicking checkout...")
        checkout_btn = page.locator('a[href*="/checkout"], button:has-text("Checkout")').first
        ...
        # Fill card details (Stripe iframe)
